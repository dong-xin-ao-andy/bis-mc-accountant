...
def get_base_delta(num_samples: int, target_delta: float) -> float:
  ...
  if get_overall_delta(num_samples, base_delta) < target_delta:
    return base_delta
  candidate = base_delta - tol
  if candidate > 0 and get_overall_delta(num_samples, candidate) < target_delta:
    return candidate
  raise ValueError(
...
def perform_calibration_from_samples(
    ...
    *,
    ...
    positive_counts: Optional[Sequence[Sequence[float]]] = None,
    negative_samples: Optional[Sequence[Sequence[float]]] = None,
    negative_counts: Optional[Sequence[Sequence[float]]] = None,
